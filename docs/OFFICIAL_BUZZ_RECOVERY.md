...
A failed or uncertain started turn also blocks the in-memory session. Restarting cannot turn incomplete work into completed work. An uncertain message or external tool action is not automatically replayed. Publication and local persistence are not one transaction; exactly-once external execution is not promised.

Provider retirement proves exit of the direct provider. It does not roll back background tools or external effects. A blocked record requires explicit reconciliation, including any surviving processes, before a conversation is reused.
